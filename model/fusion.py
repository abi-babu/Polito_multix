...
Waveform prediction
"""
class WaveformFusion(nn.Module):
    def __init__(self, waveform_length: int):
        super().__init__()
        self.waveform_length = waveform_length
        # simple learnable fusion over concatenated branch waveforms
        self.fuser = nn.Sequential(
            nn.Linear(waveform_length * 3, waveform_length),
        )

    def forward(self, branch_waveforms: Dict[str, torch.Tensor]) -> torch.Tensor:
        """
        branch_waveforms: dict of {branch_name: tensor of shape (B, T)}
        For simplicity, we fuse up to 3 branches; adjust as needed.
        """
        if not branch_waveforms:
            raise ValueError("No branch waveforms provided for fusion.")

        waves = list(branch_waveforms.values())
        # pad or truncate to 3 branches for this example
        if len(waves) == 1:
            waves = waves * 3
        elif len(waves) == 2:
            waves = waves + [waves[0]]
        ...
        x = torch.cat(waves, dim=1)  # (B, 3*T)
        fused = self.fuser(x)        # (B, T)
        return fused
...
class FusionBlock(nn.Module):
    ...
    def forward(self, output_losses, output_detections, output, fusion_sweep=False):
        ...
        for i in output_detections_copy:
            if isinstance(output_detections_copy[i], list) and isinstance(output_detections_copy[i][0], dict):
                if 'boxes' in output_detections_copy[i][0] and hasattr(output_detections_copy[i][0]['boxes'], 'numel'):
                    if output_detections_copy[i][0]['boxes'].numel() > 0:
                        good_branches.append(i)
            if i=='radar': #radar
            ...
            if i=='radar_lidar': #lidar
                ...
                for k in range(len(bboxes_3dlr)):
                    ...
                    if (0<x1<672) & (0<x2<672) & (0<y1<376) & (0<y2<376):
                        ...
                        j = j + 1
                        continue
            if i in ['rf_heatmap', 'rf_spectrogram', 'rf_fusion']:
                continue     
        ...
        for i in good_detections:
            ...
            for j in np.float6464(i[0]['boxes'].cpu()):
        ...
        return final_loss, output_detections, output
