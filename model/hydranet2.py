...
from .fusion import FusionBlock, WaveformFusion
...
class HydraFusion(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.waveform_length = config.waveform_length
    ...
    def initialize_branches(self):
        ...
        if self.config.enable_rf_heatmap:
            self.rf_heatmap_branch = RFHeatBranch(
                backbone=ResNetTail(BasicBlock, [2, 2, 2, 2], pretrained=self.config.pretrained)
            ).to(self.device)

        if self.config.enable_rf_spectrogram:
            self.rf_spectrogram_branch = RFSpectoBranch(
                backbone=ResNetTail(BasicBlock, [2, 2, 2, 2], pretrained=self.config.pretrained)
            ).to(self.device)

        if self.config.enable_rf_fusion:
            self.rf_fusion_branch = RFFusionBranch(
                backbone=ResNetTail(BasicBlock, [2, 2, 2, 2], pretrained=self.config.pretrained)
            ).to(self.device)

        # regressor for RF fused features → waveform
        self.rf_waveform_regressor = nn.Sequential(
            nn.AdaptiveAvgPool2d((1, 1)),
            nn.Flatten(),
            nn.Linear(512, self.waveform_length),
        ).to(self.device)
    ...
    def forward(self, leftcamera_x=None, rightcamera_x=None, radar_x=None, bev_lidar_x=None, l_lidar_x=None, r_lidar_x=None, radar_y=None, cam_y=None, rf_heatmap_x=None, rf_spectrogram_x=None, rf_y=None):
        ...
        if self.config.enable_rf_heatmap and rf_heatmap_x is not None:
            ...
            rf_heatmap_features = self.rf_heatmap_branch(rf_heatmap_output)
        ...
        if self.config.enable_rf_spectrogram and rf_spectrogram_x is not None:
            ...
            rf_spectrogram_features = self.rf_spectrogram_branch(rf_spectrogram_output)
    ...
    # Fuse RF features
            rf_fused_features = self.rf_fusion_branch(
                rf_heatmap_features, rf_spectrogram_features
            )

    # Convert fused RF features → waveform
            rf_waveform = self.rf_waveform_regressor(rf_fused_features)  # (B, T)

    # Optional smoothing
            rf_waveform = rf_waveform.unsqueeze(1)
            rf_waveform = F.avg_pool1d(rf_waveform, kernel_size=3, stride=1, padding=1)
            rf_waveform = rf_waveform.squeeze(1)

            #output['fused_waveform'] = rf_waveform
        ...
        for branch_index in branch_selection:  # only collect output from the branches selected by the gate module
            ...
            #RFChanges
                ...
                output_detections['rf_heatmap_features'] = rf_heatmap_features
            elif branch_index == 8:
                output_detections['rf_spectrogram_features'] = rf_spectrogram_features
            elif branch_index == 9:
                #output_detections['rf_fusion'] = rf_fused_features
                output_detections['rf_fusion_features'] = rf_fused_features
                output['fused_waveform'] = rf_waveform
