...
class WaveformBranch(nn.Module):
    def __init__(self, backbone: nn.Module, waveform_length: int) -> None:
        super().__init__()
        self.backbone = backbone
        self.regressor = nn.Sequential(
            nn.AdaptiveAvgPool2d((1, 1)),
            nn.Flatten(),
            nn.Linear(512, waveform_length),
        )

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        feats = self.backbone(x)
        waveform = self.regressor(feats)
        return waveform
...
class RFSpectoBranch(nn.Module):
    def __init__(self, backbone) -> None:
        super().__init__()
        self.backbone = backbone
...
class RFFusionBranch(nn.Module):
    def __init__(self, backbone) -> None:
        super().__init__()
        self.merge_conv = nn.Conv2d(1024, 64, kernel_size=3, stride=1, padding=1)
        self.backbone = backbone

    def forward(self, left_x: torch.Tensor, right_x: torch.Tensor) -> torch.Tensor:
        if left_x.shape[2:] != right_x.shape[2:]:
            right_x = F.interpolate(right_x, size=left_x.shape[2:], mode='bilinear', align_corners=False)
